...
%setup -q -n %{appname}-%{version} -D -T -a 13
%setup -q -n %{appname}-%{version} -D -T -a 14
...
cp -arf ./markdown %{buildroot}%{_javadir}/%{appname}/%{plugins_dir}/
...
cp -arf ./editorconfig %{buildroot}%{_javadir}/%{appname}/%{plugins_dir}/
